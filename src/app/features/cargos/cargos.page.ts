  ...
  template: `
    ...
    >
      <div form-content>
        ...
        <div class="form-group" [class.has-error]="hasFieldError('tipo')">
          ...
          @if (getFieldErrors('tipo').length > 0) {
            <div class="error-messages">
              @for (error of getFieldErrors('tipo'); track error) {
                <small class="error-text">{{ error }}</small>
              }
            </div>
          }
...
export class CargosPage {
  ...
  tableConfig: CrudTableConfig<Cargo> = {
    ...
    searchFields: ['codigo', 'nombre', 'descripcion', 'tipo'],
  };
  ...
  onModalOpened(item: Cargo | null) {
    ...
    if (item) {
      ...
      this.formData = {
        ...
        tipo: (item.tipo as 'C' | 'H') ?? null,
      };
    } else {
      // Nuevo
      this.editingCargo.set(null);
      this.formData = { codigo: '', nombre: '', descripcion: '', tipo: null };
    }
  }
