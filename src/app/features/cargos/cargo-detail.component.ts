        ...
        <div class="detail-content">
          ...
          <div class="detail-section">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                <label>Creado:</label>
                <span>{{ cargo()?.created_at }}</span>
              </div>
              <div class="detail-item">
                <label>Actualizado:</label>
                <span>{{ cargo()?.updated_at }}</span>
              </div>
            </div>
          </div>
        </div>
...
export class CargoDetailComponent {
  ...
  private loadCargo() {
    ...
    if (!idParam) {
      this.error.set(true);
      this.errorMessage.set('ID de cargo no proporcionado');
      return;
    }
    ...
    if (isNaN(id)) {
      this.error.set(true);
      this.errorMessage.set('ID de cargo inválido');
      return;
    }
    ...
    this.cargosService.getById(id).subscribe({
      next: (res: any) => {
        const data = res.data;
        this.cargo.set(data as Cargo);
        this.loading.set(false);
      },
      error: (err: any) => {
        console.error('Error cargando detalle de cargo:', err);
        this.error.set(true);
