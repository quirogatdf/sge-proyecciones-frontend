        ...
        <div class="detail-content">
          <div class="detail-section">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-item">
                <label>Sigla:</label>
                <span>{{ turno()?.sigla || 'Sin sigla' }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="detail-section">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                <label>Creado:</label>
                <span>{{ turno()?.created_at }}</span>
              </div>
              <div class="detail-item">
                <label>Actualizado:</label>
                <span>{{ turno()?.updated_at }}</span>
              </div>
            </div>
          </div>
        </div>
...
export class TurnoDetailComponent {
  ...
  private loadTurno() {
    ...
    if (!idParam) {
      this.error.set(true);
      this.errorMessage.set('ID de turno no proporcionado');
      return;
    }
    ...
    if (isNaN(id)) {
      this.error.set(true);
      this.errorMessage.set('ID de turno inválido');
      return;
    }
    ...
    this.turnosService.getById(id).subscribe({
      next: (res: any) => {
        const data = res.data;
        this.turno.set(data as Turno);
        this.loading.set(false);
      },
      error: (err: any) => {
        console.error('Error cargando detalle de turno:', err);
        this.error.set(true);
        this.errorMessage.set('No se pudo cargar el detalle del turno');
        this.loading.set(false);
      }
    });
  }
